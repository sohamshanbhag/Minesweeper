...
void allocate_grid(int grid_array[][10],int mines_array[]);
int analyse_click(int a[][10],int i,int j, ALLEGRO_DISPLAY *display, ALLEGRO_BITMAP *backgr);
int blank_box_click(int a[][10],int i,int j);
void create_grid(int mines_array[], int val);
void allocate_gridmedium(int grid_array[][10],int mines_array[]);
int win_check(int a[][10], ALLEGRO_DISPLAY *display, ALLEGRO_BITMAP *backgr, float timereg, ALLEGRO_TIMER *timer);
int drawmedium(int grid_array[][10],int screenwidth, int screenheight, ALLEGRO_DISPLAY *display, ALLEGRO_BITMAP *backgr);



int gridcreatemedium(int screenwidth, int screenheight, ALLEGRO_DISPLAY *display, ALLEGRO_BITMAP *backgr)
{
    int mines_array[10];
    int grid_array[10][10];
    create_grid(mines_array, 10);
    allocate_gridmedium(grid_array,mines_array);

    int m = drawmedium(grid_array, screenwidth, screenheight, display, backgr);
...
int drawmedium(int grid_array[][10],int screenwidth, int screenheight, ALLEGRO_DISPLAY *display, ALLEGRO_BITMAP *backgr)
{
    ...
    for(int i = 0; i < 10; i++)
    {
        for(int j = 0; j < 10; j++)
        {
            al_draw_rectangle(screenwidth/2 - 10*25 + 50*i, screenheight/2 - 10*25 + 50*j,screenwidth/2 - 10*25 + 50*(i + 1), screenheight/2 - 10*25 + 50*(j + 1),al_map_rgb(0,0,0), 1.0);
        }
    }
    ...
    while(!done)
    {
        ...
        else if(eventback.type == ALLEGRO_EVENT_MOUSE_BUTTON_DOWN)
        {
            ...
            temp = mousex - screenwidth/2 + 250;
            j = temp / 50;
            temp = mousey - screenheight/2 + 250;
            i = temp / 50;

            al_get_mouse_state(&pressbutton);
            if (pressbutton.buttons & 2)
            {
                al_draw_bitmap(Flag, screenwidth/2 - 10*25 + 50*j, screenheight/2 - 10*25 + 50*i, NULL);
                al_draw_rectangle(screenwidth/2 - 10*25 + 50*j, screenheight/2 - 10*25 + 50*i,screenwidth/2 - 10*25 + 50*(j + 1), screenheight/2 - 10*25 + 50*(i + 1),al_map_rgb(0,0,0), 1.0);
                al_flip_display();
            }
...
void allocate_gridmedium(int grid_array[][10],int mines_array[])
{
    ...
    for(int r=0;r<10;r++)
    {
        for(int c=0;c<10;c++)
        {
            ...
            if(grid_array[r-1][c+1]!=-1 && r>0 && c<10-1)grid_array[r-1][c+1]++;
            if(grid_array[r][c-1]!=-1 && c>0)grid_array[r][c-1]++;
            if(grid_array[r][c+1]!=-1 && c<10-1)grid_array[r][c+1]++;
            if(grid_array[r+1][c-1]!=-1 && r<10-1 && c>0)grid_array[r+1][c-1]++;
            if(grid_array[r+1][c]!=-1 && r<10-1)grid_array[r+1][c]++;
            if(grid_array[r+1][c+1]!=-1 && r<10-1 && c<10-1)grid_array[r+1][c+1]++;
...
int blank_box_click(int a[][10],int i,int j)
{
    ...
    if(a[i][j]!=0 && a[i][j]<10)
    {
        al_draw_filled_rectangle(screenwidth/2 - 10*25 + 50*j, screenheight/2 - 10*25 + 50*i,screenwidth/2 - 10*25 + 50*(j + 1), screenheight/2 - 10*25 + 50*(i + 1),al_map_rgb(200,200,200));
        al_draw_rectangle(screenwidth/2 - 10*25 + 50*j, screenheight/2 - 10*25 + 50*i,screenwidth/2 - 10*25 + 50*(j + 1), screenheight/2 - 10*25 + 50*(i + 1),al_map_rgb(0,0,0), 1.0);
        al_flip_display();
        c[0] = a[i][j]+48;
        c[1] = '\0';
        a[i][j]+=10;
        al_draw_text(font2, al_map_rgb(0,0,0), screenwidth/2 - 10*25 + 25 + 50*j, screenheight/2 - 10*25 + 50*i, ALLEGRO_ALIGN_CENTER, c);
        al_flip_display();
        return 0;
    }
    if(a[i][j]>=10)
...
int analyse_click(int a[][10], int i, int j, ALLEGRO_DISPLAY *display, ALLEGRO_BITMAP *backgr)
{
   ...
   // cout << "test" << endl;
    ...
    if(a[i][j]==-1)
    {
        ...
        al_draw_filled_rectangle(screenwidth/2 - 10*25 + 50*j, screenheight/2 - 10*25 + 50*i,screenwidth/2 - 10*25 + 50*(j + 1), screenheight/2 - 10*25 + 50*(i + 1),al_map_rgb(255,255,255));
        al_draw_bitmap(Mine, screenwidth/2 - 10*25 + 50*j, screenheight/2 - 10*25 + 50*i, NULL);
        al_draw_rectangle(screenwidth/2 - 10*25 + 50*j, screenheight/2 - 10*25 + 50*i,screenwidth/2 - 10*25 + 50*(j + 1), screenheight/2 - 10*25 + 50*(i + 1),al_map_rgb(0,0,0), 1.0);
        al_flip_display();}}}
        al_rest(1.5);
...
int win_check(int grid_array[][10], ALLEGRO_DISPLAY *display, ALLEGRO_BITMAP *backgr, float timereg, ALLEGRO_TIMER *timer)
{
    int m;
    int w=0;
    for(int i=0;i<10;i++){
    ...
    if(w==90)
    {
        ...
        al_clear_to_color(al_map_rgb(255,255,255));
        rectimemedium(screenwidth, screenheight, display, backgr, timereg);
        al_clear_to_color(al_map_rgb(255,255,255));
